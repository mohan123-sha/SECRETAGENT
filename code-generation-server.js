...
function writeFilesToDisk(generatedFiles, screenName) {
  ...
  try {
    ...
    if (generatedFiles.typescript) {
      ...
      console.log('✅ [File Writer] TypeScript file written:', generatedFiles.typescript.fileName);
    }
    ...
    if (generatedFiles.html) {
      ...
      console.log('✅ [File Writer] HTML file written:', generatedFiles.html.fileName);
    }
    ...
    if (generatedFiles.scss) {
      ...
      console.log('✅ [File Writer] SCSS file written:', generatedFiles.scss.fileName);
    }
...
app.post('/generate-code', async (req, res) => {
  try {
    ...
    if (testMode) {
      ...
      result = await testPipeline(figmaLayoutJSON, finalScreenName);
    } else {
      console.log('🌟 [CodeGen Server] Using full pipeline with Gemini API');
      result = await generateAngularCode(figmaLayoutJSON, finalScreenName);
    }
    ...
    const componentTsFile = `${finalScreenName.toLowerCase()}.component.ts`;
    const componentPath = path.join(fileWriteResult.componentDir, componentTsFile);
